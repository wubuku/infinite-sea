...
产品设计的需求是：假设岛屿均匀分布在地图上, 相邻的岛屿之间（从 A 岛 到离它最近的 B岛）的航行时间应该是 20 分钟——以 speed 为 5 的船/船队的速度计算。
...
假设，每个岛屿默认在地图上“占有”一个 5000 * 5000 的一个格子（“领海”），
那么，当我们确定开放 50000 * 50000 的地图区域时，大致会在这个区域放入 100 个岛屿。
反过来计算也可以，即可以根据打算开放的岛屿数量，确定要开放的地图的区域的大小。

每个岛屿的位置坐标的确认过程可以是：

* 先给每个岛屿分配默认的坐标值，把岛屿放在它所占的“格子”的中心。
* 然后，产生两个随机数，随机数的范围是 +/- 5000 * 30%（这个 30% 是举例说明，具体的随机“幅度”可以讨论）。把随机数加到默认的坐标值上，得到最终坐标值。
* 5000 * 5000 的格子，对角的直线距离是 5000 * 1.414，约等于 7070。


#### 速度的单位转换问题

明确 Ship 的 speed 属性值和以 `“最小坐标单位 / 秒”` 表示的速度值之间的转换关系：

Speed 属性 为 5 的船队，在距离为 7070 的两个岛屿之间航行，需要 20 分钟。
即大约每秒 (7070 / 1200) 个最小坐标单位，约等于 5.892 个最小坐标单位。

Speed 属性值每 +/- 1，船队的速度增加/减少 10%。所以：

| Speed 属性值 | 速度（最小坐标单位 / 秒） |
|-----------|----------------|
| 1         | 3.535          |
| 2         | 4.124          |
| 3         | 4.713          |
...
需求：每艘船航行一小时，消耗 5 个 Energy Token。

Energy Token 的小数点位数是 9。即每秒钟消耗 1388889 个 nano Energy Token。
